...
| `generate-tests-report.bat` | Windows | Runs **lint**, then **all tests** (unit with coverage + e2e), extracts results, and updates README badges. Same as: `npm run lint` then `npm run test:coverage` then `E2E_JSON_REPORT=1 npm run test:e2e` then `node test-coverage/extract-results.js` then `node test-coverage/update-readme-badges.js`. Lint runs first; if it fails, the script exits so you can fix issues locally. |
