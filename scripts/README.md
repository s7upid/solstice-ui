...
| `generate-tests-report.bat` | Windows | Runs **lint**, then **all tests** (unit with coverage + e2e), extracts results, and updates README badges. E2E runs with **max workers** (`E2E_USE_MAX_WORKERS=1`) for faster completion. Same as: `npm run lint` then `npm run test:coverage` then `E2E_JSON_REPORT=1` and `E2E_USE_MAX_WORKERS=1` then `npm run test:e2e` then `node test-coverage/extract-results.js` then `node test-coverage/update-readme-badges.js`. Lint runs first; if it fails, the script exits so you can fix issues locally. |
